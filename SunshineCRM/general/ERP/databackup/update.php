	...
	for($i=0;$i<sizeof($file);$i++)                {
		...
		if($FILE_EXEC=="1")                        {
			...
			if($PacketNameUpdate[$fileName]=="1")        {
				...
				$ReDownload .= "&nbsp;<a href=?".base64_encode("action=ExecRemoteFileServerEXESQL&FileName=$fileName&RemoteHostName=".$RemoteHostName."")."><font color=white title='额外提供功能,一般情况下面不需要使用'>SQL</font></a>";
			}
			else    {
				if($INSTALL_PACKET_NAME==$fileName)		{
					...
					if($_SESSION['LOGIN_USER_ID']=="admin")	$ReDownload .= "&nbsp;<a href=?".base64_encode("action=ExecRemoteFileServerEXESQL&FileName=$fileName&RemoteHostName=".$RemoteHostName."")."><font color=gray title='额外提供功能,一般情况下面不需要使用'>SQL</font></a>";
				}
				else			{
