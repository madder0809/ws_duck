...
.MessageBox td.stop{
   ...
   background-position:10px 10px;
	...
	function autoNavi()
	{
		...
		{
			var main=window.opener.document.getElementById('table_main');
			if(main!=null)
			{
				window.opener.document.frames('table_main').location.reload();
			}
			else
...
function page_css($add="",$title="Sunshine20",$相对路径="1")	{
	...
	function print_control(){
				...
				LODOP.ADD_PRINT_HTM(10,'5%','90%',80,'<h2 align=center>".$_SESSION['UNIT_NAME']."<br>".$html_etc[$tablename][$tablename]."</h2>');
				var strFormHtml=\"<div style='margin:auto;'>\"+strBodyStyle+document.getElementById(\"form\").innerHTML+\"</div>\";			
				LODOP.ADD_PRINT_TABLE(100,'5%','90%','100%',strFormHtml);	
...
function export_XLS($FristLineArray,$contentArray,$exportFileName,$sumArray)					{
	...
	for($m=1;$m<=$allcount;$m++)		{
		$worksheet1->set_row($m, 16);
		$i=0;
		foreach($FristLineArray as $key=>$val)		{
			$Element = $contentArray[$m-1][$key];
	...
	foreach($FristLineArray as $key=>$val)		{
		...
		else
		{
			...
			foreach ($sumArray as $sumkey=>$sumval)
			{
				if($sumkey==$key)
				{
					$worksheet1->write_string($allcount+1, $i, $sumval, $sumer);
					$flag=true;
					break;
				}
			}
