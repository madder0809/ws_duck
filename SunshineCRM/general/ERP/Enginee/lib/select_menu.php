...
function getCustomerRoleByUser($sql,$fieldname)		{
	...
	$sql = $sql." and ((($fieldname='".$_SESSION['LOGIN_USER_ID']."' or $fieldname in (select user_id from user where dept_id in (".$subdeptrole."))) and datascope>0) or datascope='1')";
	return $sql;
}
//权限判断
function getCustomerRoleByCustID($sql,$fieldname)		{
	...
	$sql = $sql." and $fieldname in (select rowid from customer where ((sysuser='".$_SESSION['LOGIN_USER_ID']."' or sysuser in (select user_id from user where dept_id in (".$subdeptrole."))) and datascope>0) or datascope='1')";
...
function getKucunByUserid($sql,$userid,$storeid="storeid")
{
	...
	{
		$sql = $sql." and $storeid in (select rowid from stock where leverno='是' or user_id like '%".$userid.",%')";
	}
	return $sql;
}
