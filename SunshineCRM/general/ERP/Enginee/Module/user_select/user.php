...
else
{
    if ( $MANAGE_FLAG )
    {
        $query = "SELECT USER_ID,USER_NAME from user where USER_PRIV='".$USER_PRIV."' and DEPT_ID!=0";
    }
    else
    {
        $query = "SELECT USER_ID,USER_NAME from user where USER_PRIV='".$USER_PRIV."' and DEPT_ID!=0 and NOT_LOGIN!='1'";
    }
     if($MODULE_ID==1)
    	$query=getRoleByUser($query,"USER_ID");
    $query=$query."  order by UID,USER_NAME";	
