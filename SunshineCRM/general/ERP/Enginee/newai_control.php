...
switch($action_type)		{
	case 'add':
		...
		if($action_add=='data')	{
				...
				//直接调转到另一个页面
				if($fields['tablename']=="buyplanmain")			
				{
					print "<script>location='DataQuery/productFrame.php?tablename=buyplanmain_detail&deelname=采购单明细&rowid=".$_POST['AUTO_INCREMENT_billid']."'</script>";
					exit;
				}
