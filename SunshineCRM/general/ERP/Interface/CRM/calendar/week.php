...
echo "</select>     </td>\r\n           <td align=\"right\">\r\n        <input type=\"button\"  value=\"新建日程\" class=\"SmallButton\" onClick=\"new_cal('".date("H")."')\" title=\"创建新的日程，以便提醒自己\">\r\n        <select name=\"VIEW\" class=\"SmallSelect\" onchange=\"set_view();\">\r\n          <option value=\"day\">&nbsp;日列表</option>\r\n          <option value=\"week\" selected>&nbsp;周列表</option>\r\n          <option value=\"month\">&nbsp;月列表</option>\r\n        </select>\r\n </td>\r\n    </tr>\r\n   </form>\r\n  </table>\r\n";
$CUR_TIME = date( "Y-m-d H:i:s", time( ) );
$query = "SELECT * from calendar where USER_ID='".$user_id."' and ((CAL_TIME>='".date( "Y-m-d", $WEEK_BEGIN )." 00:00:00' and CAL_TIME<='".date( "Y-m-d", $WEEK_END )." 23:59:59') or (END_TIME>='".date( "Y-m-d", $WEEK_BEGIN )." 00:00:00' and END_TIME<='".date( "Y-m-d", $WEEK_END )." 23:59:59') or (CAL_TIME<'".date( "Y-m-d", $WEEK_BEGIN )." 00:00:00' and END_TIME>'".date( "Y-m-d", $WEEK_END )." 23:59:59')) order by CAL_TIME";
$rs=$db->Execute($query);
$rs_a=$rs->GetArray();
for ($i=0;$i<sizeof($rs_a);$i++)
{
    ...
    $CONTENT = cutStr($rs_a[$i]['CONTENT'],11);
    ...
    if(compare_time($END_TIME, date("Y-m-d 23:59:59",strtotime($CAL_TIME)))>0)
    {
    	$showBeginDate=date("d日H:i",strtotime($CAL_TIME));
    	$showEndDate=date("d日H:i",strtotime($END_TIME));	
    }
    $CONTENT = htmlspecialchars( $CONTENT );
    $CONTENT = "<div id=\"cal_".$CAL_ID."\" title='".$CAL_TITLE."' style='width:100%;'>".$showBeginDate."-".$showEndDate.( "<br><a href='javascript:my_note(".$CAL_ID.");' style='color:" ).$STATUS_COLOR.";'>".$CONTENT."</a>";
    ...
    if(compare_time($CAL_TIME,date("Y-m-d",$WEEK_BEGIN))>0)
    ...
    for($j=0;$j<$days;$j++)
    {
    	...
    	if(compare_time($nextDay,date("Y-m-d",$WEEK_BEGIN))>0 && compare_time($nextDay,date("Y-m-d 23:59:59",$WEEK_END))<0)
    		$CAL_ARRAY[date( "w", strtotime( $nextDay ) )][24] .= $CONTENT;
    }
}

echo "\r\n  <table class=\"TableBlock\" width=\"100%\" align=\"center\">\r\n    <tr align=\"center\" class=\"TableHeader\">\r\n      <td class=\"TableControl\" width=\"9%\"><a href=\"javascript:display_front();\">0-6点</a></td>\r\n      <td width=\"13%\"><b><a href=\"day.php?BTN_OP=a&YEAR=";
...
for ( ; $I < 25; ++$I )
{
    ...
    echo $I==24?"跨天":"<a href=\"javascript:new_cal('$I')\">".$I.":00</a>";
    echo "</td>\r\n      <td width=\"13%\"";
    if ( $DATE == $WEEK_BEGIN )
    {
        echo " class=\"TableContent\"";
