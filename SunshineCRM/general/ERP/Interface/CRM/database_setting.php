...
<tr class="TableData">
 <td colspan="6" align=center height=32>
    <input type="button"  value="删除废弃单据" class="BigButton" onClick="javascript:
    var beginvalue=document.getElementById('begintime').value;
    var endvalue=document.getElementById('endtime').value;
    var param='action=DeleteFeiQiDan&begintime='+beginvalue+'&endtime='+endvalue;
    if(confirm('你真的要进行此项操作么?'))
    	location='?'+param;" title="废弃单据删除">
 </td>
 <td colspan="6" align=left width=80%>删除采购单、合同、订单、店面销售单中的已撤销单据。<br>开始时间: <INPUT  class=SmallInput maxLength=20  name=开始时间 value=<?php echo date("Y-m-d",mktime(1,1,1,date('m'),date('d')-21,date('Y')))?>  id="begintime" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true})">
