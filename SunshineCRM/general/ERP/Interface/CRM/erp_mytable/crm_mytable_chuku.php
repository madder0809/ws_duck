...
$sql = "select * from stockoutmain where state='未出库'";
   ...
   for($i=0;$i<count($rs_a);$i++){
	   ...
	   $customerName='';
   		if($outtype=="销售出库")	
   		{
			...
			$sellinfo =returntablefield("sellplanmain","billid",$rs_a[$i]['dingdanbillid'],"supplyid,zhuti,billtype");
		...
		{
			$sellinfo=returntablefield("buyplanmain", "billid", $rs_a[$i]['dingdanbillid'], "supplyid,zhuti");
			$supplyid=$sellinfo['supplyid'];
			$zhuti=$sellinfo['zhuti'];
	   ...
	   $module_body .= "<tr class=\"TableBlock\">
						...
						<td valign=\"Middle\" align=\"right\"><a href=../../JXC/$url?".base64_encode("action=view_default&billid=".$rs_a[$i]['dingdanbillid'])." target=_blank>".$zhuti."</a></td>
						<td valign=\"Middle\" align=\"right\"><a href=../../JXC/stockoutmain_newai.php?action=init_default_search&searchfield=billid&searchvalue=".$billid." target=_blank>".$state."</a></td>
