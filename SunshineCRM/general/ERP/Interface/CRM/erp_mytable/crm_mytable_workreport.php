...
$sql = "select a.*,b.USER_NAME,b.UID from workreport a inner join user b on a.createman=b.user_id where ($user_priv='1' and state='未审核') or (b.dept_id=$dept_id and $user_priv<b.user_priv and state='未审核') order by createtime desc limit 0,$max_count";
...
if(count($rs_a)>0){
  ...
  for($i=0;$i<count($rs_a);$i++){
	 ...
	 $module_body .= "<tr class=TableBlock>
						...
						<td valign=Middle align=left><a href=../../CRM/workreport_newai.php?".base64_encode("action=view_default&id=$id")." target=_blank>".$rs_a[$i]['workdate']."</a></td>
						<td valign=Middle align=right>".$rs_a[$i]['createtime']."</td>
						<td valign=Middle align=right><a href=../../CRM/workreport_newai.php?".base64_encode("action=shenhe&id=$id")." target=_blank>审核</a></td>
