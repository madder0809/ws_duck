...
$sql = "select * from v_supplyownmoney where 1=1 ";
$sql=$sql." order by own desc limit 0 ,$max_count";
...
if(count($rs_a)>0){
   for($i=0;$i<count($rs_a);$i++){
	   ...
	   $huikuan=number_format($rs_a[$i]['yufukuan'],2);
	   ...
	   $module_body .= "<tr class=\"TableBlock\">
	   					<td><img src=\"../images/arrow_r.gif\" align=\"absmiddle\"><a href=../../JXC/supply_newai.php?".base64_encode("action=view_default&ROWID=".$rs_a[$i]['rowid'])." target=_blank title='$title'>".$supplyname."</td>
						<td><a href='../../JXC/v_supplyownmoney_mingxi.php?".base64_encode("supplyid=$rowid")."' target='_blank'>单数:".$num."</a></td>
						<td valign=\"Middle\" align=\"left\">金额:".$own."</td>
						<td valign=\"Middle\" align=\"left\">预付:".$huikuan."</td>
						<td valign=\"Middle\" align=\"left\">未付:".$pay_own."</td>
					  </tr>";
