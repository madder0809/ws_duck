...
$sql="SELECT  a.prodid,a.prodname,a.prodguige,a.prodxinghao,a.proddanwei,SUM(a.num) as num,SUM(a.jine) as sumjine FROM sellplanmain_detail a JOIN sellplanmain b ON (b.billid=a.mainrowid) WHERE b.user_flag>0 and  b.createtime>='".$start_time."' and b.createtime<='".$end_time."' GROUP BY a.prodid order by ".$order;
//exit($sql);
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();

$head=array("prodid"=>"产品ID","prodname"=>"产品名称","prodguige"=>"规格","prodxinghao"=>"型号","proddanwei"=>"单位","num"=>"销售数量","sumjine"=>"销售总金额");
...
<table class=TableBlock align=center width=100%>
	<thead>
		...
		<tr>
			<td colspan="11" class="TableHeader">&nbsp;产品销售汇总</td>
		</tr>
	</thead>
...
chart_index.setDataXML("<graph bgcolor='e1f5ff' caption='产品销售汇总' subCaption='精确到百位（四舍五入）' numberPrefix='' formatNumberScale='1' decimalPrecision='2' baseFontSize='14' numberSuffix='万元'><?php foreach ($rs_a as $row){echo "<set name='".$row['prodname']."' value='".($row['sumjine']/10000)."'/>";}?></graph>");
