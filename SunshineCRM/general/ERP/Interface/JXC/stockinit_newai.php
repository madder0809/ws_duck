...
if($_GET['action']=='edit_default2')
{
	...
	if(sizeof($rs_detail)>0)
	{
		print "<script language=javascript>alert('错误：已存在库存初始化记录，不能重复初始化');window.history.back(-1);</script>";
    	exit;
	}
	$sql="delete from store_init where storeid=$storeid and flag=0";
	$db->Execute($sql);
...
if($_GET['action']=='save')
{
	...
	try 
	{
		...
		for($i=0;$i<sizeof($rs_detail);$i++)
		{
			...
			$jine=$_POST["jine_".$rs_detail[$i]['id']];
			$allnum=$allnum+$num;
