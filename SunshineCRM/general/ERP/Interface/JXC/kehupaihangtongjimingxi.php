...
$sql="SELECT a.ifpay,a.oddment,a.billid,c.supplyname,a.zhuti,d.`机会主题` AS jihui,a.totalmoney,a.huikuanjine,a.fahuojine,a.kaipiaojine,b.USER_NAME,a.user_flag,a.createtime,a.fahuostate,a.kaipiaostate FROM sellplanmain a left JOIN `user` b on a.user_id=b.USER_ID left JOIN customer c on a.supplyid=c.ROWID left JOIN crm_chance d on a.chanceid=d.`编号` where a.user_flag>0 and ".$where." a.supplyid='".$supplyid."' AND a.createtime>='".$start_time."' AND a.createtime<='".$end_time."'";
//exit($sql);
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();
$i=0;
...
$head=array("supplyname"=>"客户名称","billid"=>"单据ID","zhuti"=>"主题","totalmoney"=>"总金额","oddment"=>"去零金额","huikuanjine"=>"回款金额","fahuojine"=>"发货金额","kaipiaojine"=>"开票金额","createtime"=>"创建日期","USER_NAME"=>"创建人","user_flag"=>"订单状态","ifpay"=>"付款状态","fahuostate"=>"发货状态","kaipiaostate"=>"开票状态");
$headtype=array("supplyname"=>"string","billid"=>"char","zhuti"=>"char","totalmoney"=>"float","oddment"=>"float","huikuanjine"=>"float","fahuojine"=>"float","kaipiaojine"=>"float","createtime"=>"char","USER_NAME"=>"char","user_flag"=>"char","ifpay"=>"char","fahuostate"=>"char","kaipiaostate"=>"char");
$title="客户排行明细";
