...
class CaiWu
{
	...
	function updatesellplanmainFlag($dingdanid)
	{
		...
		if($user_flag!=-1)
		{
			if($sellplaninfo['billtype']==3)
			{
				...
				{
					$ifchuku=false;
					$billid=returntablefield("stockoutmain", "dingdanbillid", $dingdanid, "billid","state","已出库","outtype","销售出库");
					if($billid!='')
						$ifchuku=true;
					$billid=returntablefield("stockinmain", "caigoubillid", $dingdanid, "billid","state","已入库","intype","退货入库");
					if($billid!='')
						$ifchuku=true;
					if($ifpay==2 && $ifchuku)
						$user_flag=2;
	...
	function deleteShoukuanReocord($id)
	{
		...
		if($shoukuan!=0)
		{
			if($accountid=='0')
			{
				...
				$this->db->Execute($sql);
				$sql="insert into accesspreshou (id,customerid,curchuzhi,jine,opertype,guanlianbillid,createman,createtime) values(
				".$id.",".$customerid.",".$yuchuzhi.",".$shoukuan.",'货款收取',".$dingdanbillid.",'".$_SESSION['LOGIN_USER_ID']."','".date("Y-m-d H:i:s")."')";
				$this->db->Execute($sql);
			}
			else 
			{
	...
	function deleteFukuanReocord($id)
	{
		...
		if($accountid=='0')
		{
			...
			$this->db->Execute($sql);
			$sql="insert into accessprepay (id,supplyid,curchuzhi,jine,opertype,guanlianbillid,createman,createtime) values(
			".$id.",".$supplyid.",".$yufukuan.",".$fukuan.",'货款支付',".$caigoubillid.",'".$_SESSION['LOGIN_USER_ID']."','".date("Y-m-d H:i:s")."')";
			$this->db->Execute($sql);
		}
		else 
		{
