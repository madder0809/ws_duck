...
function CountRecJine(id)
{
	...
	if(recnum+hasnum>oldnum)
	{
		...
		form1.elements("num_"+id).value=oldnum-hasnum;
		return;
	}
...
<?php 
	$sql = "select * from buyplanmain_detail where mainrowid=".$rowid;
	$rs = $db->Execute($sql);
	$rs_a = $rs->GetArray();
...
?>
            <tr class=<?php echo $class?>>
                ...
                <td align="right" ><?php echo $rs_a[$i]['zhekou']*100?>%</td>
                ...
                <input type="hidden" name="price_<?php echo $rs_a[$i]['id']?>" value="<?php echo $rs_a[$i]['price']?>">
                <input type="hidden" name="zhekou_<?php echo $rs_a[$i]['id']?>" value="<?php echo $rs_a[$i]['zhekou']?>">
                <input type="hidden" name="recjine_<?php echo $rs_a[$i]['id']?>" value="<?php echo $jine?>"> 
                <input type="hidden" name="prodid_<?php echo $rs_a[$i]['id']?>" value="<?php echo $rs_a[$i]['prodid']?>"> 
