...
<?php 
}else if($tablename=='sellplanmain_detail_color')
{
...
function saveAndClose()
{
	...
	$.ajax({ 
		  type:'GET', 
		  url:'buyplanmain_mingxi_update.php?tablename=sellplanmain_detail&rowid=<?php echo $id?>&action=colorinput&colorarray=' + colorarray.toString(), 
		  dataType: 'text', 
		  cache:false,
		  async: false,
		  success:function(data) 
...
<tr>
	...
	{
		$num=0;
		$sql="select * from sellplanmain_detail_color where id=$id and color=".$rs_b[$i]["id"];
		$rs = $db->Execute($sql);
		$rs_c = $rs->GetArray(); 
		if(sizeof($rs_c)==1)
			$num=$rs_c[0]['num'];
