...
function accesspreshou_value( $fieldvalue, $fields, $i )
{
		...
		if($opertype=='货款收取' || $opertype=='欠款收取' || $opertype=='收押金')
		{
			...
			else if($billinfo['billtype']==2)
			{
				...
				$fieldvalue="<a href='sellonemain_newai.php?action=view_default&billid=".$billinfo['billid']."' target='_blank'>".$billinfo['zhuti']."</a>";
		}
		else 	
			$fieldvalue='';
...
function accesspreshou_value_PRIV( $fieldvalue, $fields, $i )
{
	...
	if(stripos($fields['value'][$i]['opertype'],'货款收取'))			
	{
