...
if($_GET['action']=="edit_default2")			
{
	    ...
	    //插入新订单
	    $sql = "insert into sellplanmain (billid,zhuti,user_id,supplyid,chanceid,totalmoney,plandate,zuiwanfahuodate,beizhu,linkman,address,mobile,createtime,billtype) values(".
	    $billid.",'".$zhuti."','".$_SESSION['LOGIN_USER_ID']."',".$customerid.",'".$chance."',".$jine.",'".date("Y-m-d")."','".date("Y-m-d",strtotime(date("Y-m-d")." +30 days")).
	    "','".$beizhu."','".$jieshouren."','".$address."','".$tel."','".date("Y-m-d H:i:s")."',2)";
		...
		for($i=0;$i<sizeof($rs_detail);$i++)
		{
			...
			$sql = "insert into sellplanmain_detail (id,prodid,prodname,prodguige,prodxinghao,proddanwei,price,zhekou,num,mainrowid,jine) values(".$maxid.",'".
    		$rs_detail[$i]['prodid']."','".$rs_detail[$i]['prodname']."','".$rs_detail[$i]['prodguige']."','".$rs_detail[$i]['prodxinghao'].
    		"','".$rs_detail[$i]['proddanwei']."',".$rs_detail[$i]['price'].",".$rs_detail[$i]['zhekou'].",".$rs_detail[$i]['num'].",".$billid.",".$rs_detail[$i]['price']*$rs_detail[$i]['zhekou']*$rs_detail[$i]['num'].")";
    		$db->Execute($sql);
