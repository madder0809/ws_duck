...
global $db;
$sql="select `a`.`billid`,`a`.`zhuti`,`a`.`user_id`,`a`.`chanceid`,`a`.`createtime`,`a`.`qianyueren`,`a`.`fahuostate`,`b`.`ROWID` AS `rowid`,`b`.`supplyname`,`a`.`createtime`,`a`.`totalmoney`,`a`.`huikuanjine`,`a`.`oddment`,`b`.`yuchuzhi`,(`a`.`totalmoney` - `a`.`huikuanjine` - `a`.`oddment`) AS `own` ,`d`.`USER_NAME` AS `createman` ,`c`.`name` AS `typename` from (`sellplanmain` `a` join `customer` `b` on`a`.`supplyid` = `b`.`ROWID` left join `sellbilltype` `c` on `a`.`billtype` = `c`.`id` left join `user` `d` on`a`.`user_id` = `d`.`USER_ID`) where (`a`.`ifpay` < 2 and `a`.`user_flag` = 1 and `a`.`supplyid`=".$_GET['supplyid']." )";
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();
$yuchuzhi=0;
if(sizeof($rs_a)>0)
	$yuchuzhi=$rs_a[0]['yuchuzhi'];
$head=array("billid"=>"单号","supplyname"=>"客户","typename"=>"订单类型","zhuti"=>"主题","createman"=>"创建人","createtime"=>"创建日期","totalmoney"=>"总金额","huikuanjine"=>"回款金额","oddment"=>"去零金额","own"=>"应收金额");
$headtype=array("billid"=>"string","supplyname"=>"string","typename"=>"string","zhuti"=>"string","qianyueren"=>"string","createtime"=>"string","totalmoney"=>"float","huikuanjine"=>"float","oddment"=>"float","own"=>"float");
$title="应收款明细";
...
<p align="center"><input type="button" class="SmallButton" value=" 打印 "
	...
	onclick="location='v_yingshoukuanhuizong_mingxi.php?supplyid=<?php echo $_GET[supplyid];?>&out_excel=true';">
