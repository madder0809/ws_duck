...
//print_r($_GET);
$sql="select a.supplyid,`b`.`supplyname` AS `name`,sum(`a`.`totalmoney`) AS `totalmoney`,sum(`a`.`huikuanjine`) AS `huikuanjine`,sum(`a`.`fahuojine`) AS `fahuojine`,sum(`a`.`kaipiaojine`) AS `kaipiaojine`,count(0) AS `num`,b.ROWID from `sellplanmain` `a` left join `customer` `b` on a.supplyid=b.ROWID  where a.user_flag>0 and  a.createtime>='".$start_time."' and a.createtime<='".$end_time."'  group by `a`.`supplyid` order by ".$order." ".$sc;
//print($sql);
$rs=$db->Execute($sql);
$rs_a = $rs->GetArray();
$head=array("name"=>"客户名称","num"=>"订单数量","totalmoney"=>"交易金额","huikuanjine"=>"回款金额","fahuojine"=>"发货金额","kaipiaojine"=>"开票金额");
$headtype=array("name"=>"string","num"=>"int","totalmoney"=>"float","huikuanjine"=>"float","fahuojine"=>"float","kaipiaojine"=>"float");
$title="客户排行";
...
foreach($rs_a as $row)
	{
		...
		foreach ($head as $key=>$val)
		{
			...
			switch($val){
				...
				case "订单数量":	
					print "<a target='_blank' href='kehupaihangtongjimingxi.php?supplyid=".$row['supplyid']."&start_time=".$start_time."&end_time=".$end_time."'>".$row[$key]."</a>";
					break;	
				case "交易金额":	
					print "<a target='_blank' href='kehupaihangtongjimingxi.php?supplyid=".$row['supplyid']."&start_time=".$start_time."&end_time=".$end_time."'>".number_format($row[$key],2,'.',',')."</a>";
					break;
				case "回款金额":
					print "<a target='_blank' href='kehupaihangtongjimingxi.php?supplyid=".$row['supplyid']."&start_time=".$start_time."&end_time=".$end_time."&where=huikuan'>".number_format($row[$key],2,'.',',')."</a>";;
					break;
				case "发货金额":
					print "<a target='_blank' href='kehupaihangtongjimingxi.php?supplyid=".$row['supplyid']."&start_time=".$start_time."&end_time=".$end_time."&where=fahuo'>".number_format($row[$key],2,'.',',')."</a>";;
					break;
				case "开票金额":
					print "<a target='_blank' href='kehupaihangtongjimingxi.php?supplyid=".$row['supplyid']."&start_time=".$start_time."&end_time=".$end_time."&where=kaipiao'>".number_format($row[$key],2,'.',',')."</a>";;
					break;
...
chart_index.setDataXML("<graph bgcolor='e1f5ff' caption='客户交易金额排行' subCaption='精确到百位（四舍五入）' numberPrefix='' formatNumberScale='1' decimalPrecision='2' baseFontSize='14' numberSuffix='万元'><?php foreach ($rs_a as $row){echo "<set name='".$row['name']."' value='".($row['totalmoney']/10000)."'/>";}?></graph>");
